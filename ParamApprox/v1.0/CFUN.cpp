...
arma::dcube simulateWFM_logisticnorm_arma(const arma::dmat& mean, const arma::dcube& variance, const int& int_gen, const int& lst_gen, const arma::uword& sim_num) {
  ...
  frq_pth.row(0) = exp(Y.row(0)) / (ones_cube.row(0) + exp(Y.row(0)) + exp(Y.row(1)) + exp(Y.row(2)));
  frq_pth.row(1) = exp(Y.row(1)) / (ones_cube.row(0) + exp(Y.row(0)) + exp(Y.row(1)) + exp(Y.row(2)));
  frq_pth.row(2) = exp(Y.row(2)) / (ones_cube.row(0) + exp(Y.row(0)) + exp(Y.row(1)) + exp(Y.row(2)));
  frq_pth.row(3) = exp(ones_cube.row(0)) / (ones_cube.row(0) + exp(Y.row(0)) + exp(Y.row(1)) + exp(Y.row(2)));
  return frq_pth;
}
...
List approximateMoment_logisticnorm_arma(const arma::dmat& m, const arma::dcube& V, const int& int_gen, const int& lst_gen, const arma::uword& sim_num) {
  ...
  arma::dcube frq_pth = simulateWFM_logisticnorm_arma(m, V, int_gen, lst_gen, sim_num);
  ...
  for(arma::uword k = 1; k < arma::uword(lst_gen - int_gen) + 1; k++) {
    ...
    arma::dmat cov_mat = arma::cov(frq_smp.t(), frq_smp.t());
